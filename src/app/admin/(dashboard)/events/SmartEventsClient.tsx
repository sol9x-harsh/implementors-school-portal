...
import { createEvent, getEvents, updateEvent, deleteEvent } from '@/lib/actions/admin.actions';
...
export default function SmartEventsClient({ initialEvents }: { initialEvents: EventItem[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.set('linkedExternalEvents', JSON.stringify([{ title, url, startDate: '', endDate: '' }]));
            ...
            <div className='space-y-6'>
              <div className='flex items-center gap-3 pb-4 border-b border-purple-border/30'>
                ...
                <div>
                  ...
                  <p className='text-[10px] text-purple-muted-foreground font-bold uppercase tracking-wider mt-1.5'>
                    {editingId ? `Modifying: ${editingId}` : 'Define the external resource for students'}
                  </p>
                </div>
              </div>
              ...
              <div className='grid grid-cols-1 sm:grid-cols-2 gap-6'>
                ...
                <div className='space-y-2.5'>
                  ...
                  <Input
                    id='title'
                    name='title'
                    placeholder='e.g. Science Hackathon 2026'
                    value={title}
              ...
              <div className='space-y-2.5'>
                ...
                <textarea
                  ...
                  placeholder='What should students know before visiting this link?'
                    ...
                    {audienceOptions.map((opt) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex-1 py-3 px-3 rounded-xl flex items-center justify-center gap-2.5 transition-all duration-300
                        ...
                        >
                          <Icon className={`w-4 h-4 ${isActive ? 'text-purple-primary' : 'text-purple-muted-foreground'}`} />
                          <span className={`text-[11px] font-black uppercase tracking-widest ${isActive ? 'text-purple-primary' : ''}`}>
                            {opt.label}
                          </span>
                        </button>
                      );
                ...
                <div className='flex items-center gap-3 pt-2'>
                  ...
                  <span className='text-[11px] font-bold text-purple-muted-foreground uppercase tracking-widest'>
                    ...
                    <span className='text-purple-primary'>
                      {audience === 'cohort' ? cohort : 'All Students'}
                    </span>
                  </span>
                </div>
          ...
          <table className='w-full'>
            <thead>
              <tr className='text-left border-b border-purple-border/20'>
                <th className='pb-4 text-[10px] font-black text-purple-muted-foreground uppercase tracking-widest px-4'>Activity</th>
                <th className='pb-4 text-[10px] font-black text-purple-muted-foreground uppercase tracking-widest px-4'>Audience</th>
                <th className='pb-4 text-[10px] font-black text-purple-muted-foreground uppercase tracking-widest px-4'>Deadline</th>
                <th className='pb-4 text-[10px] font-black text-purple-muted-foreground uppercase tracking-widest px-4 text-right'>Actions</th>
              </tr>
            </thead>
                ...
                {events.map((event) => (
                  ...
                  >
                    <td className='py-5 px-4'>
                      <div className='flex items-center gap-3'>
                        <div className='w-10 h-10 rounded-xl bg-purple-primary/5 flex items-center justify-center group-hover:bg-white transition-colors'>
                          <LinkIcon className='w-4 h-4 text-purple-primary/40' />
                        </div>
                        <div>
                          <p className='font-bold text-purple-foreground text-sm leading-tight'>
                            {event.title}
                          </p>
                          <p className='text-[10px] text-purple-muted-foreground font-medium truncate max-w-[200px] mt-1'>
                            {event.url}
                          </p>
                        </div>
                      </div>
                    ...
                    <td className='py-5 px-4'>
                      <div className='flex items-center gap-2'>
                        {event.targetAudience.includes('all') ? (
                          ...
                          <div className='flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-indigo-500/10 text-[9px] font-black text-indigo-600 uppercase tracking-widest'>
                            <GraduationCap className='w-3 h-3' /> {event.targetAudience[0]}
                          </div>
                        )}
                      </div>
                    </td>
