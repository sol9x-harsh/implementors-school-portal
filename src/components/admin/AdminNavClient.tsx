    ...
    <div className='space-y-5'>
      {sections.map((section) => (
        <div key={section.label}>
          {/* Section label */}
          <p
            className='text-[9px] font-black uppercase tracking-[0.22em] px-3 mb-1.5 text-purple-muted-foreground'
          >
            {section.label}
          </p>
                ...
                <Link key={item.name} href={item.href} className='block'>
                  <motion.div
                    ...
                    className={`relative flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] transition-colors duration-150 group
                      ${
                        isActive
                          ? 'text-purple-primary bg-purple-primary/10 border-l-2 border-purple-primary'
                          : 'text-purple-muted-foreground hover:text-purple-foreground hover:bg-purple-secondary/20 border-l-2 border-transparent'
                      }`}
                  >
                    {/* Hover background */}
                    ...
                    {/* Label */}
                    <span
                      className='font-medium tracking-wide truncate relative z-10'
                      style={{ fontSize: '13px' }}
                    >
                      {item.name}
                    </span>
